...
/**
 * The TreasureHunt class extends the PApplet class and defines the settings(), setup(),
 * draw() and loadGameSettings() methods along with its helpers.
 * 
 * @author Ayan Deep Hazra
 *
 */
public class TreasureHunt extends PApplet {
  ...
  public void draw() {
    ...
    for (int i = 0; i < this.gameObjects.size(); i++) {
      ...
      // if action is not null, call the act method with the ArrayList
      if (a != null) {
        a.act(gameObjects);
      }
   ...
   *              object - y: the starting y position (as an int) for this object - message: a 
   *              string of text to display when this object is clicked - name of the object
  ...
  private DroppableObject loadNewDroppableObject(String[] parts) {
    ...
    DroppableObject newObject =
        new DroppableObject(name, x, y, (VisibleObject) dropTarget,
