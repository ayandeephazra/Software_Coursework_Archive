...
public class TreasureHunt extends PApplet {
  ...
  */
  ...
  /*
  Action a = new Action("What a cute stuffed koala! It looks like a real one!");
  
  ClickableObject v1 = new ClickableObject("koala", 350, 65, a);
  gameObjects.add(v1);
  
  DraggableObject d2 = new DraggableObject("key", 20, 20);
  gameObjects.add(d2);

  VisibleObject v3 = new VisibleObject("chest", 365, 400);
  gameObjects.add(v3);
  
  Action b = new Action("Open sesame!");
  DroppableObject d3 = new DroppableObject("key", 70, 70, v3, b);
  gameObjects.add(d3);
  */
    ...
    for (int i = 0; i < this.gameObjects.size(); i++) {
        ...
        for (int k = 0; k < this.gameObjects.size(); k++) {
            if (!gameObjects.get(k).isActive()) {
                gameObjects.remove(gameObjects.get(k));
                k--;
            }
        }
        if (a != null) {
