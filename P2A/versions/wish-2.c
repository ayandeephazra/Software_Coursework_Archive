    ...
    while (fgets(str, 500, f) != NULL)
    {
        ...
        else if (strcmp(commands[0], "loop") == 0)
        {
            loop_mode(tokenCount, commands);
             ...
             else if (strcmp(commands[0], "p1.sh") == 0)
        {
            loop_mode(tokenCount, commands);
        }
        /*
...
*/
        else
        {
            for (int i = 0; i < strlen(commands[0]); i++)
            {
                ...
                else if (commands[0][i] == '.' && currentRunPathInvoke == 1)
                {

                    if (commands[0][i + 1] == 's' && commands[0][i + 2] == 'h')
                    {
                        char *argvee[5];
                        argvee[0] = "./wish";
                        argvee[1] = commands[0];
